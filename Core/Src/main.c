...
{
    int out_idx = 0;
    int start = fft_queue_index;   // oldest frame after queue is full

    /*
     * IMPORTANT:
     *
     * Your Python CSV shape was:
     *   256 rows x 16 columns
     *
     * Python reshape(-1) flatten order is row-major:
     ...
     *
     * So STM32 input order must be:
     *   freq0_time0, freq0_time1, ..., freq0_time15,
     *   freq1_time0, freq1_time1, ...
     */
...
{
    /*
     * Normalize exactly like Python StandardScaler:
    ...
    for (int i = 0; i < MODEL_INPUT_SIZE; i++)
    {
        ...
        ai_input_data[i] = (input_4096[i] - scaler_mean[i]) / scale;
    }
    ...
    float prob = ai_output_data[0];
    int pred = (prob >= 0.5f) ? 1 : 0;

    int prob_int = (int)(prob * 1000.0f);

    printf("Prediction=%d prob=%d.%03d frame=%lu\r\n",
           pred,
           prob_int / 1000,
           prob_int % 1000,
...
void ProcessFFT(int32_t *input)
{
    ...
    for (int i = 0; i < FFT_SIZE; i++)
    {
        float32_t w = 0.5f - 0.5f * cosf(2.0f * 3.1415926f * i / (FFT_SIZE - 1));
        fft_input[i] *= w;
    }
