...
{
    float A = 100000.0f;   // amplitude

    for(int i = 0; i < FFT_SIZE; i++)
    {
        float t = (float)i / FS;
        TestBuf[i] = (int32_t)(A * sinf(2.0f * 3.1415926f * 1000.0f * t));
    }
}
...
void ProcessFFT(int32_t *input)
{
    ...
    for (int i = 0; i < FFT_SIZE; i++)
    {
        float w = 0.5f - 0.5f * cosf(2 * 3.1415926f * i / (FFT_SIZE - 1));
        fft_input[i] *= w;
    }
...
//  GenerateTestSignal();
//  ProcessFFT(TestBuf);
//  arm_max_f32(fft_mag, FFT_SIZE/2, &maxValue, &fftindex);
//  float freq = (float)fftindex * FS / FFT_SIZE;
  /* USER CODE END 2 */
